...
import ru.Natro.worldmanager.utils.Metrics.DrilldownPie;
import ru.Natro.worldmanager.utils.Metrics.SimplePie;
import ru.Natro.worldmanager.utils.Metrics.SingleLineChart;
...
public class CustomMetricsManager {
	...
	public static void loadMetrics() {
		...
		SimplePie xboxAuth = new SimplePie("onlineMode", new Callable<String>() {
			...
			@Override
			public String call() throws Exception {
				Config cfg = new Config(new File(Server.getInstance().getDataPath() + "server.properties"), Config.PROPERTIES);
				return String.valueOf(Boolean.parseBoolean(cfg.getString("xbox-auth", "false")));
			}
		});
